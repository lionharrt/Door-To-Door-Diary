...
var propTypes = {
  ...
  autoHidePagination: _propTypes2.default.bool
};

var pageItem = function pageItem(idx, page, onPageChange) {
...
var ellipsisItem = function ellipsisItem(idx) {
  return _react2.default.createElement(
    'li',
    { key: 'ellipsis-' + idx, className: 'page-item' },
    _react2.default.createElement(
      'div',
      { className: 'page-link' },
      '...'
...
var pagesBeforeCurrent = 5;
var pagesAfterCurrent = 5;
var currentPageRange = function currentPageRange(page, pageCount) {
  var rangeCount = Math.min(pageCount - 2, pagesBeforeCurrent + pagesAfterCurrent + 1);
  var firstPageMinValue = 1;
  var firstPageMaxValue = pageCount - 1 - rangeCount;
  var firstPage = page - pagesBeforeCurrent;
  firstPage = Math.max(firstPage, firstPageMinValue);
  firstPage = Math.min(firstPage, firstPageMaxValue);
  return _lodash2.default.range(firstPage, firstPage + rangeCount);
};
var middlePages = function middlePages(page, pageCount, onPageChange) {
  var pageRange = currentPageRange(page, pageCount);
  var ret = [];
  if (pageRange[0] > 1) ret.push(ellipsisItem(0));
  ret.push(pageRange.map(function (idx) {
    return pageItem(idx, page, onPageChange);
  }));
